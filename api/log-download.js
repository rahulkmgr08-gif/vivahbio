async function supabaseRequest(path, options = {}) {
  ...
  if (!res.ok) {
    throw new Error(
      data?.message || data?.hint || text || "Supabase request failed"
    );
  }
  ...
  try {
    ...
    if (!paymentId || !["pdf", "jpg"].includes(fileType)) {
      return res.status(400).json({
        error: "payment_id and valid file_type are required."
      });
    }

    // payment_id must be the Supabase payments.id UUID returned
    // by /api/verify-payment after successful verification.
    const payments = await supabaseRequest(
      `payments?select=id,profile_id,status&` +
    ...
    return res.status(500).json({
      error: err.message || "Could not log download."
    });
  }
};
